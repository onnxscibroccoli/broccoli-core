...
class WorkflowGovernor:
    """
    Supervise workflow execution freshness, queue activity, and executor
    heartbeat.
    """
    ...
    def __init__(
        ...
        self.last_snapshot: Optional[WorkflowSnapshot] = None
        ...
        for topic in WORKFLOW_TOPICS:
            try:
                self.bus.subscribe(topic, self._on_workflow_event)
            except Exception:
                pass
    ...
    def _classify(self) -> Tuple[str, str]:
        if self.last_event_at is None:
            return "WORKFLOW_WARNING", "No workflow events observed yet."

        age = max(0.0, time.time() - self.last_event_at)
        if age <= self.warning_seconds:
            return "WORKFLOW_OK", "Workflow stream is fresh."
        if age <= self.critical_seconds:
            return "WORKFLOW_WARNING", "Workflow stream is aging."
        return "WORKFLOW_CRITICAL", "Workflow stream is stale."

    def collect(self) -> WorkflowSnapshot:
    ...
    def _emit(self, topic: str, snapshot: WorkflowSnapshot, severity: str = "INFO", detail: str = "") -> None:
        payload = {
            "timestamp": snapshot.timestamp,
            "source": "workflow_governor",
            "event": topic,
            "severity": severity,
        ...
        try:
            self.bus.publish(topic, payload, source="WorkflowGovernor")
        except Exception:
            pass

        outfile = self.processed_dir / f"workflow_{snapshot.timestamp}.json"
        try:
    ...
    def _transition_events(self, previous: Optional[WorkflowSnapshot], current: WorkflowSnapshot):
        if previous is None:
        ...
        if previous.status != current.status:
            ...
            if current.status == "WORKFLOW_OK":
                events.append(("WORKFLOW_RECOVERED", "INFO", current.note))
            elif previous.status == "WORKFLOW_OK" and current.status in {"WORKFLOW_WARNING", "WORKFLOW_CRITICAL"}:
                events.append(("WORKFLOW_DEGRADED", self._severity_for(current.status), current.note))
        elif current.status == "WORKFLOW_OK" and (time.time() - self.last_emit_at) >= self.heartbeat_interval_seconds:
            events.append(("WORKFLOW_HEARTBEAT", "INFO", current.note))

        return events

    def run_once(self) -> WorkflowSnapshot:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Broccoli Core Workflow Governor")
