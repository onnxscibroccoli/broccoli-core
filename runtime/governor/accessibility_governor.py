...
class AccessibilityGovernor:
    """
    Supervise accessibility telemetry freshness and publish standardized
    accessibility health events.

    This governor is intentionally lightweight. It observes accessibility
    topics already flowing through the EventBus and evaluates whether that
    stream is healthy, warning, or stalled.
    """
    ...
    def __init__(
        ...
        self.last_snapshot: Optional[AccessibilitySnapshot] = None
        self.last_emit_at: float = 0.0
        self.running = False
        self.event_counts = defaultdict(int)

        for topic in A11Y_TOPICS:
            try:
                self.bus.subscribe(topic, self._on_a11y_event)
            except Exception:
                pass
    ...
    def _on_a11y_event(self, event: Any) -> None:
    ...
    def _classify(self) -> Tuple[str, str]:
        ...
        if self.last_event_at is None:
            return "ACCESSIBILITY_WARNING", "No accessibility events observed yet."

        age = max(0.0, time.time() - self.last_event_at)
        if age <= self.warning_seconds:
            return "ACCESSIBILITY_OK", "Accessibility stream is fresh."
        if age <= self.critical_seconds:
            return "ACCESSIBILITY_WARNING", "Accessibility stream is aging."
        return "ACCESSIBILITY_CRITICAL", "Accessibility stream is stale."

    def collect(self) -> AccessibilitySnapshot:
    ...
    def _emit(self, topic: str, snapshot: AccessibilitySnapshot, severity: str = "INFO", detail: str = "") -> None:
        payload = {
            "timestamp": snapshot.timestamp,
            "source": "accessibility_governor",
            "event": topic,
            "severity": severity,
        ...
        try:
            self.bus.publish(topic, payload, source="AccessibilityGovernor")
        except Exception:
            pass

        outfile = self.processed_dir / f"accessibility_{snapshot.timestamp}.json"
        try:
    ...
    def _transition_events(self, previous: Optional[AccessibilitySnapshot], current: AccessibilitySnapshot):
        if previous is None:
        ...
        if previous.status != current.status:
            ...
            if current.status == "ACCESSIBILITY_OK":
                events.append(("ACCESSIBILITY_RECOVERED", "INFO", current.note))
            elif previous.status == "ACCESSIBILITY_OK" and current.status in {"ACCESSIBILITY_WARNING", "ACCESSIBILITY_CRITICAL"}:
                events.append(("ACCESSIBILITY_DEGRADED", self._severity_for(current.status), current.note))
        elif current.status == "ACCESSIBILITY_OK" and (time.time() - self.last_emit_at) >= self.heartbeat_interval_seconds:
            events.append(("ACCESSIBILITY_HEARTBEAT", "INFO", current.note))

        return events

    def _severity_for(self, status: str) -> str:
        return {
            "ACCESSIBILITY_OK": "INFO",
            "ACCESSIBILITY_WARNING": "WARNING",
            "ACCESSIBILITY_CRITICAL": "CRITICAL",
        }.get(status, "INFO")

    def run_once(self) -> AccessibilitySnapshot:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Broccoli Core Accessibility Governor")
    parser.add_argument("--loop", action="store_true", help="Run continuously")
    parser.add_argument("--interval", type=int, default=60, help="Polling interval in seconds")
    args = parser.parse_args()

    governor = AccessibilityGovernor()
    if args.loop:
