...
class SupervisorSnapshot:
    ...
    status: str
    policy_decision: str
    components: List[SupervisorComponent] = field(default_factory=list)
    note: str = ""
...
class GovernorSupervisor:
    """
    Top-level orchestrator for all governor artifacts.

    It aggregates the current governor outputs into one supervisory decision
    and can request recoveries via registered handlers.
    """
    ...
    def __init__(
        ...
        self.last_snapshot: Optional[SupervisorSnapshot] = None
        ...
        for topic in SUPERVISOR_TOPICS:
            try:
                self.bus.subscribe(topic, self._on_supervisor_event)
            except Exception:
                pass
    ...
    def _on_supervisor_event(self, event: Any) -> None:
    ...
    def _normalize(self, raw: Any) -> str:
        ...
        if value in NORMALIZE:
            return NORMALIZE[value]
        if value.endswith("_ok"):
            return "healthy"
        if value.endswith("_warning"):
            return "warning"
        if value.endswith("_critical"):
            return "critical"
        if value.endswith("_recovered"):
            return "healthy"
        if value.endswith("_degraded"):
            return "warning"
        return "warning"
    ...
    def _status_from_artifact(self, data: Dict[str, Any], required: bool, age_seconds: Optional[float]) -> Tuple[str, str]:
        raw = None
        for key in ("policy_decision", "status", "overall_status", "event", "severity"):
    ...
    def collect(self) -> SupervisorSnapshot:
        components: List[SupervisorComponent] = []
        warnings = 0
        critical = 0
        missing = 0
        ...
        for name, pattern, required in self.COMPONENT_PATTERNS:
            ...
            if latest is None:
                status, note = self._status_from_artifact({}, required, None)
                components.append(
                    SupervisorComponent(
                        name=name,
                        status=status,
                        ...
                        note=note,
                        required=required,
                        details={"pattern": pattern, "required": required, "source": "missing"},
                    )
                )
                if required:
            ...
            status, note = self._status_from_artifact(data, required, age)
            ...
            components.append(
                SupervisorComponent(
                    ...
                    note=note,
                    required=required,
                    details={"pattern": pattern, "required": required, "source": latest.name},
                )
            )
        ...
        if critical >= self.critical_threshold or missing > 0:
            status = "SUPERVISOR_CRITICAL"
            policy_decision = "restart_or_escalate"
        elif warnings >= self.warning_threshold:
            status = "SUPERVISOR_WARNING"
            policy_decision = "monitor_and_prepare_recovery"
        else:
            status = "SUPERVISOR_OK"
            policy_decision = "steady_state"

        note = f"{status} | required={len(required_names)} warnings={warnings} critical={critical} missing={missing}"
        ...
        return SupervisorSnapshot(
            ...
            metadata={
                ...
                "required_components": required_names,
                "warnings": warnings,
                "critical": critical,
                "missing": missing,
            },
    ...
    def _severity_for(self, status: str) -> str:
        return {
            "SUPERVISOR_OK": "INFO",
            "SUPERVISOR_WARNING": "WARNING",
            "SUPERVISOR_CRITICAL": "CRITICAL",
            "SUPERVISOR_RECOVERY_REQUIRED": "CRITICAL",
        }.get(status, "INFO")

    def _event_for_status(self, status: str) -> Tuple[str, str]:
        if status == "SUPERVISOR_OK":
            return "SUPERVISOR_OK", "INFO"
        if status == "SUPERVISOR_WARNING":
            return "SUPERVISOR_WARNING", "WARNING"
        if status == "SUPERVISOR_CRITICAL":
            return "SUPERVISOR_CRITICAL", "CRITICAL"
        return "SUPERVISOR_WARNING", "WARNING"

    def _publish(self, topic: str, snapshot: SupervisorSnapshot, severity: str = "INFO", detail: str = "") -> None:
        payload = {
            "timestamp": snapshot.timestamp,
            "source": "governor_supervisor",
            "event": topic,
            "severity": severity,
        ...
        try:
            self.bus.publish(topic, payload, source="GovernorSupervisor")
        except Exception:
            pass

        out = self.processed_dir / f"governor_supervisor_{snapshot.timestamp}.json"
        try:
    ...
    def _invoke_recovery(self, component_name: str, context: Dict[str, Any]) -> Dict[str, Any]:
        handler = self.handlers.get(component_name) or self.handlers.get("*")
        if handler is None:
            return {
                "success": True,
                "action": "request_only",
                "detail": f"No handler registered for {component_name}; request published only.",
            }
        ...
        if isinstance(result, dict):
            ...
            result.setdefault("detail", f"Custom handler executed for {component_name}.")
            return result

        return {
            "success": bool(result is None or result is True),
            "action": "custom_handler",
            "detail": f"Custom handler executed for {component_name}.",
        }
    ...
    def run_once(self) -> SupervisorSnapshot:
        ...
        if self.last_snapshot is None:
            self._publish(topic, snapshot, severity=severity, detail=snapshot.note)
        elif self.last_snapshot.status != snapshot.status:
            self._publish(topic, snapshot, severity=severity, detail=snapshot.note)
            if self.last_snapshot.status == "SUPERVISOR_OK" and snapshot.status in {"SUPERVISOR_WARNING", "SUPERVISOR_CRITICAL"}:
                self._publish("SUPERVISOR_DEGRADED", snapshot, severity=severity, detail=snapshot.note)
            if self.last_snapshot.status in {"SUPERVISOR_WARNING", "SUPERVISOR_CRITICAL"} and snapshot.status == "SUPERVISOR_OK":
                self._publish("SUPERVISOR_RECOVERED", snapshot, severity="INFO", detail=snapshot.note)

        if snapshot.status == "SUPERVISOR_CRITICAL":
            self._publish("SUPERVISOR_RECOVERY_REQUIRED", snapshot, severity="CRITICAL", detail=snapshot.note)

            for component in snapshot.components:
                if component.required and component.status in {"critical", "missing"}:
                    context = {
                    ...
                    self._publish(
                        ...
                        snapshot,
                        severity="WARNING",
                        detail=f"Restart requested for governor {component.name}",
                    )
                    try:
                        self._invoke_recovery(component.name, context)
                    except Exception:
                        pass
        elif snapshot.status == "SUPERVISOR_OK" and (time.time() - self.last_emit_at) >= self.heartbeat_interval_seconds:
            self._publish("SUPERVISOR_HEARTBEAT", snapshot, severity="INFO", detail=snapshot.note)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Broccoli Core Governor Supervisor")
    ...
    if args.loop:
        ...
        print(json.dumps(supervisor.run_once().to_dict(), indent=2))
