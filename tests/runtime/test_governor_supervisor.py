...
class GovernorSupervisorTest(unittest.TestCase):
    ...
    def test_fresh_governor_artifacts_produce_ok(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for name, payload in (
                ...
                ("production_1.json", {"status": "PRODUCTION_OK"}),
            ):
                self._write_artifact(root, name, age_seconds=1, payload=payload)

            bus = FakeBus()
            emitted = []
            gov = GovernorSupervisor(
                bus=bus,
                root=root,
                warning_seconds=10,
                critical_seconds=30,
            ...
            self.assertEqual(snap.status, "SUPERVISOR_OK")
            self.assertTrue(any(evt[0] == "SUPERVISOR_OK" for evt in bus.events))
            self.assertTrue(any(e["event"] == "SUPERVISOR_OK" for e in emitted))

    def test_stale_required_governor_artifact_triggers_critical(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            ...
            self.assertEqual(snap.status, "SUPERVISOR_CRITICAL")
            self.assertTrue(any(evt[0] in {"SUPERVISOR_CRITICAL", "SUPERVISOR_RECOVERY_REQUIRED"} for evt in bus.events))
